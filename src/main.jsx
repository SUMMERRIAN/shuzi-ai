...
const mistakeQuickTasks = {
  analyzeMistake: {
    label: "AI分析错题",
    prompt: (grade) => `对面是一个${grade || "当前年级"}的学生，请讲解这道题目。`,
  },
  generateSimilar: {
    label: "AI生成类似题",
    prompt: (grade) =>
      `这是一道${grade || "当前年级"}的题目，请根据这道题生成1-3道处理方法类似、思路结构相近的训练题，帮助学生巩固同一种解题模型或思维方法。每道题包含题目、答案和简要解析。`,
  },
  analyzePaper: {
    label: "AI分析试卷",
    prompt: (grade) =>
...
function App() {
  ...
  async function runMistakeWorkspaceAi() {
    ...
    if (!mistakeTaskType && !mistakePrompt.trim()) {
      setAiNotice({ page: activePage, message: "请点击一个快捷任务，或者在输入框里写清楚你的要求。" });
      return;
    }
    ...
    try {
      ...
      const taskPrompt =
        mistakeTaskType && typeof mistakeQuickTasks[mistakeTaskType]?.prompt === "function"
          ? mistakeQuickTasks[mistakeTaskType].prompt(mistakeDraft.grade)
          : "";
  ...
  function downloadMistakePdf() {
    ...
    if (!mistakeResult) {
      setAiNotice({ page: activePage, message: "请先完成一次AI处理，再下载PDF。" });
      return;
    }
    const title = mistakeResult.title || mistakeQuickTasks[mistakeTaskType]?.label || "错题专项分析";
    const reportHtml = renderMistakeReportHtml(mistakeResult);
    const win = window.open("", "_blank");
    if (!win) {
      setAiNotice({ page: activePage, message: "浏览器拦截了打印窗口，请允许弹窗后再下载PDF。" });
      return;
    }
      ...
      <html>
        <head>
          ...
          <style>
            ...
            body { margin: 0; color: #17211b; font-family: "Microsoft YaHei", "PingFang SC", Arial, sans-serif; line-height: 1.75; }
            main { max-width: 760px; margin: 0 auto; }
            h1 { margin: 0 0 18px; font-size: 26px; line-height: 1.25; }
            h2 { margin: 24px 0 10px; padding-top: 10px; border-top: 1px solid #dfe5da; font-size: 18px; }
            h3 { margin: 18px 0 8px; font-size: 15px; }
            p, li { font-size: 13.5px; white-space: pre-wrap; }
            p { margin: 0 0 10px; }
            ul { margin: 0 0 12px 20px; padding: 0; }
            .summary { padding: 10px 12px; border-left: 4px solid #008653; background: #f2f8f4; font-weight: 700; }
        ...
        <section className="mistake-teacher-card">
          <div className="mistake-teacher-grid">
            ...
            <article>
              <span>2</span>
              <strong>考点定位</strong>
              <p>{teacher.test_point || "先确认这道题考查的核心知识点。"}</p>
            </article>
            <article>
              <span>3</span>
              <strong>已知条件</strong>
...
function renderMistakeReportHtml(report) {
  ...
  return `
    ...
    ${report.summary ? `<p class="summary">${escape(report.summary || "")}</p>` : ""}
    ${teacher.question_restate || standardSteps.length ? `
      <h2>标准讲解</h2>
      <h3>题目在问什么</h3><p>${escape(teacher.question_restate || "")}</p>
      <h3>考点定位</h3><p>${escape(teacher.test_point || "")}</p>
